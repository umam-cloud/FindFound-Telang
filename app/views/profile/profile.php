<main class="max-w-6xl mx-auto px-6 py-10 md:py-14">
    <div class="flex flex-col md:flex-row gap-8 mb-16 bg-white p-8 md:p-10 rounded-[2rem] shadow-sm border border-gray-100">
        <div class="w-40 h-40 md:w-48 md:h-48 rounded-[2rem] overflow-hidden shrink-0 bg-gray-100 border border-gray-200">
            <img src="<?= !empty($data['profile']['foto_profil']) ? BASEURL . '/img/profile/' . $data['profile']['foto_profil'] : 'https://api.dicebear.com/9.x/adventurer-neutral/svg?seed=Felix'. urlencode($data['nama'] ?? 'User') .'&backgroundColor=b6e3f4' ?>" alt="Foto Profil" class="w-full h-full object-cover">
        </div>
        <div class="flex flex-col justify-center w-full">
            ...
            <h1 class="text-3xl md:text-4xl font-extrabold text-gray-900 mb-4 tracking-tight"><?=$data['profile']['nama']?></h1>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            <?php foreach ($data['post'] as $post): 
                ...
                if ($status == 'aktif') {
                    ...
                    $btnText = 'selesai';
                    $badgeClass = 'bg-gray-200 text-gray-600 hover:bg-gray-300';
                }?>
                <div class="bg-white rounded-[2rem] overflow-hidden shadow-sm border border-gray-100 flex flex-col group hover:shadow-md transition-shadow">
                    ...
                    <div class="p-6 flex-grow flex flex-col">
                        <div class="flex justify-between items-center mb-4">
                            <span class="<?=$badgeClass?> text-[9px] font-bold uppercase tracking-widest px-2 py-1 rounded"><?=$btnText?></span>
                            <span class="text-[10px] text-gray-400 font-bold"><?=$tanggal?></span>
                        </div>
    ...
    <div class="relative rounded-[2rem] overflow-hidden shadow-lg h-64 md:h-[340px] flex items-center p-10 md:p-16 group">
        ...
        <div class="relative z-10 text-white max-w-xl">
            ...
            <p class="text-sm md:text-base text-teal-50 font-medium">Anda telah membantu mengembalikan barang berharga tahun ini.</p>
        </div>
    </div>
