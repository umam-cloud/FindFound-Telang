...
<main class="max-w-6xl mx-auto px-6 py-12">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
       ...
       <?php foreach($data as $index => $post):
            ...
            if ($status == 'aktif') {
                $btnText = ($jenis == 'hilang') ? 'Hubungi Pemilik' : 'Klaim Barang';
        ...
        ?>
            <?php if ($posisi == 0): ?>
                <div class="bg-white rounded-[2rem] overflow-hidden shadow-sm border border-gray-100 lg:col-span-2 flex flex-col md:flex-row relative">
                    <div class="<?=$badgeClass?> absolute top-6 left-6 z-10 text-[10px] font-bold px-3 py-1 rounded-sm uppercase tracking-wider"><?=$jenis?></div>
                    ...
                    <div class="p-8 md:p-10 flex flex-col justify-center w-full md:w-1/2">
                        <div class="flex items-center gap-4 text-xs text-gray-500 mb-3">
                            <span class="flex items-center gap-1"><svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"></path>
                        </svg><?=$tanggal?></span>
                        </div>
            ...
            <div class="bg-white rounded-[2rem] overflow-hidden shadow-sm border border-gray-100 lg:col-span-2 flex flex-col-reverse md:flex-row relative">
                <div class="<?=$badgeClass?> absolute top-6 right-6 md:right-auto md:left-[55%] z-10 text-[10px] font-bold px-3 py-1 rounded-sm uppercase tracking-wider"><?=$jenis?></div>
                <div class="p-8 md:p-10 flex flex-col justify-center w-full md:w-1/2">
                    <div class="flex items-center gap-4 text-xs text-gray-500 mb-3">
            ...
            <?php else:?>
                <div class="bg-white rounded-[2rem] overflow-hidden shadow-sm border border-gray-100 relative flex flex-col">
                    <div class="<?=$badgeClass?> absolute top-6 left-6 z-10 text-[10px] font-bold px-3 py-1 rounded-sm uppercase tracking-wider"><?=$jenis?></div>
                    <img src="<?=BASEURL?>/img/postingan/<?=$post['file_path']?>" alt="Gambar Barang" class="w-full h-56 object-cover">
