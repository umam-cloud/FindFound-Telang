                ...
                <div class="mt-auto">
                    ...
                    <?php if (!$pemosting):?>
                        <div class="mt-4 text-center">
                            ...
                            <?php else: ?>
                                <a href="javascript:void(0);" onclick="Swal.fire({title: 'Oops!', text: 'Silakan login terlebih dahulu untuk dapat melaporkan postingan ini.', icon: 'warning', confirmButtonColor: '#006D77'}).then(() => window.location.href = '<?= BASEURL; ?>/auth');" class="text-xs font-bold text-gray-400 hover:text-red-500 transition-colors flex items-center justify-center gap-1.5 mx-auto">
                                    <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"></path></svg>
                                    Laporkan Postingan
                                </a>
                            <?php endif; ?>
    ...
    <div id="reportModal" class="fixed inset-0 z-50 hidden flex items-center justify-center">
        ...
        <div id="reportModalContent" class="bg-white rounded-[2rem] p-8 max-w-md w-full mx-6 relative z-10 shadow-2xl transform scale-95 opacity-0 transition-all duration-300 ease-out max-h-[90vh] overflow-y-auto no-scrollbar">
            ...
            <form action="<?= BASEURL; ?>/postingan/laporkan/<?= $data['id']; ?>/<?= $data['kode_postingan']; ?>" method="POST">
